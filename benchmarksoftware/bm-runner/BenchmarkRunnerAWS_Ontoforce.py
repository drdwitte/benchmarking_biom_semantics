...
def run(no):
    print(["./sparql-query-bm-2.1.0/cmd/benchmark", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-t", "1200", "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "--log-file", "runner.log"])
    if requiresAuth:
        print ("requires auth")
        return subprocess.Popen(["./sparql-query-bm-2.1.0/cmd/benchmark", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "-t", "1200", "--username", user, "--password", pwd,  "--log-file", "runner.log"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    else:
        return subprocess.Popen(["./sparql-query-bm-2.1.0/cmd/benchmark", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "-t", "1200", "--log-file", "runner.log"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def hasTriple(sparql, triple):
    ...
    if requiresAuth:
        ...
        response = requests.get(sparql.endpoint + '?format=json&query=%s' % urllib.quote_plus(query))
