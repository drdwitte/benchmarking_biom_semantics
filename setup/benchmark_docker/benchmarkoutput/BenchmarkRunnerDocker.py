...
def run(no):
    print(["./benchmark", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "--log-file", "runner.log"])
    if requiresAuth:
        print ("requires auth")
        return subprocess.Popen(["./", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "--username", user, "--password", pwd,  "--log-file", "runner.log"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    else:
        return subprocess.Popen(["./benchmark", "-m", "./%s" % queryListTxt, "-r", str(runs), "-o", str(outliers), "-w", str(warmups), "-q", endpoint, "-p", str(parallell), "-c", "./%s_%s_%s.csv" % (resultsFile, no, str(int(time.time()))), "--log-file", "runner.log"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
