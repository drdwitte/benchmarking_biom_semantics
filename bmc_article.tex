...
\begin{figure}[ht!]
	\centering
 	...
 	\caption{\csentence{Average runtime per query template for different engine hardware and configurations.}
 		For every engine and for every template category the effect of improving hardware and configuration is consistent and uniform. The only exception being Virtuoso for which the \emph{Optimized} setting performs worse as compared to the \emph{Default} setting.
 		}
