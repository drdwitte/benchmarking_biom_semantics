...
%Results I: \label{sec:tradeoffs}
%Results II: \label{sec:runtimefactors}
%Results III: \label{sec:realworld}
...
\begin{enumerate}
	...
	This will be mostly addressed in the methods section. Reliability is the focus of result sections `Query Result Completeness' and `Benchmark Error Analysis', which deal with errors and query completeness. 
	...
	In section~\ref{sec:tradeoffs} we will demonstrate that the choice of engine depends on the size of the dataset. In section~\ref{sec:runtimefactors} we will investigate which engine works best for certain query types. How completely different solutions can be compared is the subject of the subsection `Benchmark Cost' of section~\ref{sec:realworld} . There we also compare the results on artificial data with a real-world dataset.
	...
	In the section~\ref{sec:runtimefactors} we will demonstrate that query runtime results cannot be interpreted without taking into account the \emph{query context}: server load, availability, caching effects, etc. all have an impact on the individual runtimes.
...
\begin{itemize}
	...
	\item Self-provided Docker images: Blazegraph~\cite{dockerblazegraph}, \\ HDT-Fuseki~\cite{hdtfusekidocker} 
...
\subsection{Approaches to Linked Data at Scale}
\label{sec:tradeoffs}
%\input{resultsI.tex}
...
\subsubsection{Runtime Performance for Different Dataset Scales}
...
\subsubsection{RFI-Optimized Configuration}
\label{subsec:rfi}
...
\subsubsection{Query Response Times}
%response times
...
\subsection{Query Runtime Analysis in Depth}
...
\subsubsection{Query runtimes for different template types}
...
\subsubsection{Single versus Multi-client stress testing}
\label{subsec:load}
...
\subsubsection{The Role of Caching in Query Runtime Results}
...
\subsubsection{Query Result Completeness}
...
\subsection{Real-world Life Sciences Benchmark Results}
\label{sec:realworld}
...
\subsubsection{Decision Tree Analysis of Query Features}
\label{subsec:dtrees}
