...
Our initial results with the 4 \emph{Vendor} systems showed large differences in runtime performance~\cite{de2016big}. After consulting with the database vendors, it turned out that this can be attributed to our choice of running the systems as-is, which we name the \emph{None} configuration. We decided to re-run these benchmarks using two strictly defined configurations: \emph{Default} and \emph{Optimized}. All configuration options are defined in Table~\ref{table:configs}. 
...
\begin{table}[htbp!]
	...
	\scalebox{0.9}{
		\begin{tabular}{l|l}
			...
			\hline
			\emph{None}    & Results reported in initial work were obtained \\
			&  modifying none of the preset configurations. \\
			\emph{Default} & Applying the recommended settings from the vendor \\
