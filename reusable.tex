...
\begin{itemize}
	...
	\item Self-provided Docker images: Blazegraph~\cite{dockerblazegraph}, \\ \todo{HDT-Fuseki: drdwitte-rdfbenchmarking-setup-endpoints-FusekiHDT}
...
The query events can also be used to study query correctness since they contain the number of results per query and a flag for (un)successful query execution. For the Ontoforce benchmark however, almost half of the queries are \sql{count} queries, for which the result count doesn't provide any guarantees on correctness. 
